...
function protocolCollisionServices() {
  return {
    ...
    ftpThird: {
      protocol: 'ftps',
      upstream: 'ftps://shared.example.com:990',
      hostnames: ['shared.example.com'],
      auth: { type: 'plugin' },
      policy: { default: 'require_approval' },
...
test('HTTP proxy rejects FTP services before approval or forwarding', async () => {
  let approvalCalls = 0;
  const config = {
    ...
    services: {
      files: {
        protocol: 'ftps',
        upstream: 'ftps://127.0.0.1:990',
        auth: { type: 'plugin', pluginPath: 'ftp-password' },
        policy: { default: 'require_approval' },
        ftp: { allowPrivateTarget: true, tlsMode: 'implicit' },
      },
    },
  ...
  const approvalManager = {
    getStatus: () => ({}),
    async checkApproval() { approvalCalls++; },
  };
  const audit = { getRecentRequests: () => [], logRequest() { throw new Error('FTP was audited as HTTP'); } };
  const server = http.createServer(createProxy(config, approvalManager, audit));
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  const { port } = server.address();
  try {
    const response = await fetch(`http://127.0.0.1:${port}/files/path`, {
      headers: { 'x-clawguard-key': 'agent-key', connection: 'close' },
    });
    assert.equal(response.status, 404);
    assert.deepEqual(await response.json(), {
      error: 'FTP/FTPS services are not available over the HTTP proxy',
    });
