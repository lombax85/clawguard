# Experimental OpenSSH gateway sidecar
...
Create the exact host-side bind source before starting Compose:
...
an upstream private key. The entrypoint copies it into the unprivileged
account's home with strict ownership and mode.
...
The SSH sidecar has a Compose profile, so ordinary `docker compose up` keeps
running the existing HTTP-only ClawGuard service. Start both containers with:
...
ephemeral agent-socket path. Wrapper exit reports only the exit status and
requests lease cleanup; the TTL and ClawGuard shutdown cleanup are additional
backstops.
...
- **`authorized keys file not found` / `is empty`** — create the host file at
  `./data/ssh-gateway/authorized_keys` before starting the profile.
